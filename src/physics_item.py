...
class PhysicsItem():
    def __init__(self, item, position, velocity = None, pickupDelay = 100):
    ...
    def RenderImage(self):
        self.image = pygame.transform.scale(surface_manager.items[self.item.ID].copy(), (int(commons.BLOCKSIZE * 1.414 * self.itemScale), int(commons.BLOCKSIZE * 1.414 * self.itemScale)));
    ...
    def Update(self):
        ...
        if self.item.ID not in entity_manager.clientPlayer.unPickupableItems:
            if self.pickupDelay <= 0:
                if abs(self.position[0] - entity_manager.clientPlayer.position[0]) < commons.BLOCKSIZE * 4 and abs(self.position[1] - entity_manager.clientPlayer.position[1]) < commons.BLOCKSIZE * 4:
                    ...
                    if entity_manager.clientPlayer.rect.colliderect(self.rect):
                        itemAddData = entity_manager.clientPlayer.GiveItem(self.item.ID, amnt = self.item.amnt, unique = self.item.hasPrefix, item = self.item);
                            
                        if itemAddData[0]:
                            entity_manager.physicsItems.remove(self);
                            entity_manager.AddRecentPickup(self.item.ID, self.item.amnt, self.item.tier, entity_manager.clientPlayer.position, unique = self.item.hasPrefix, item = self.item);
                            
                            if commons.SOUND:
                                if self.item.ID >= 21 and self.item.ID <= 24:
                                    sound_manager.sounds[23].play();
                                else:
                                    sound_manager.sounds[19].play();
                            return;
                        else:
                            self.item.amnt = itemAddData[1];
                        return;
            else:
                self.pickupDelay -= 1;
